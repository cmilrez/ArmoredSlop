...
func physics_update(node, delta):
	var distance = node.global_position.distance_squared_to(targeting.global_position)
	var direction = targeting.global_position - node.global_position
	direction = direction.normalized()
	if distance < 900.0: # 30
		node.move_direction = -direction
	elif distance < 3600.0: # 50
